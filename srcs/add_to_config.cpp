...
static int	consumeSemicolon(Tokeniser &tokeniser) {
	...
	if (tokeniser.peek().type != TOKEN_END && tokeniser.peek().type != TOKEN_COMMENT) {
		log_error<std::string>("Unexpected token after directive: " + tokeniser.peek().value + tokeniser.getLineContext());
		return -1;
	}
	return 0;
}
...
int	config::addAllowedMethods(Tokeniser &tokeniser, locationConfig &location) const {
	while (tokeniser.peek().type == TOKEN_WORD) {
		if (tokeniser.peek().value != "GET" && tokeniser.peek().value != "POST" && tokeniser.peek().value != "DELETE" && tokeniser.peek().value != "HEAD") {
			log_error<std::string>("Invalid HTTP method: " + tokeniser.peek().value + tokeniser.getLineContext());
			return -1;
		}
